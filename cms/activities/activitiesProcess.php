...
{    
    ...
    require './../auth/dbConnect.php';
    $query = "SELECT * FROM activitylog LEFT JOIN staffs ON activitylog.staffID = staffs.staffID ";
    ...
    // NOT SEARCHING AND CURRENT ROLE FETCHING 
        ...
        //fetch data for Literary Editor 
        if($_SESSION['position'] == 'Literary Editor'){
            $query .= 'WHERE activitylog.staffID ="'.$staffID.'" ORDER BY timestamp DESC  ';
        }   
        // fetch data for News Editor 
        if($_SESSION['position'] == 'News and Current Affairs Editor' ){
            $query .= 'WHERE activitylog.staffID ="'.$staffID.'" ORDER BY timestamp DESC  ';
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        <div class="px-6 py-4 whitespace-normal w-max">
            <div class="text-sm text-gray-900">' . date("F j Y", strtotime($row["timestamp"])) . '</div>
            <div class="text-sm text-gray-900">' . date("g:i A", strtotime($row["timestamp"])) . '</div>
        </div>
        ';
        ...
        <div class="px-6 py-4 w-max whitespace-nowrap line-clamp-1">
            <div class="block text-sm font-medium text-gray-900 line-clamp-1"> ' . $row["description"] . '  </div>
        ...
        $sub_array[] = '
        <div class="px-6 py-4 w-max whitespace-nowrap line-clamp-1">
            <div class="block text-sm font-medium text-gray-900 line-clamp-1"> ' . $row["action"] . '  </div>
